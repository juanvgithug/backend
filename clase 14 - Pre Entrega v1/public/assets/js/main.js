...
formAddProduct.addEventListener('submit', e => {
    ...
    const producto = {
        ...
        description: formAddProduct[2].value,
        code: formAddProduct[3].value,
        stock: formAddProduct[4].value,
        thumbnail: formAddProduct[5].value
    }
    ...
    .then(response => response.json())
        .then(
            function (data) {
                                ...
                                let messages = '<div class="ResponseText">';
                                messages += '<b>' + "Ruta" + '</b>: ' + data.descripcion + '<br>';
                                messages += '<b>' + "Status" + '</b>: ' + data.error + '<br>';
                                messages += '<b>' + "Metodo" + '</b>: ' + data.metodo + '<br>';
                messages += '</div>';
                fswal("Respuesta obtenida", messages);
                console.log(data, response);
        }
...
socket.on('productos', productos => {
    console.log("socket.on(productos)", productos);
